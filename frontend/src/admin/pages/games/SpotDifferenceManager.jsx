                    ...
                    >
                      ...
                      <div className="space-y-6 pt-8 border-t border-stone-100">
                        ...
                        <div className="space-y-2.5">
                          ...
                          <div className="preview-media-box">
                            {formData.secondImageUrl ? (
                              <div className="relative group">
                                <img
                                  src={resolveImageUrl(formData.secondImageUrl)}
                                  alt="Target"
                                  className="w-full h-[220px] object-cover rounded-3xl border border-rose-50"
                                />
                                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity rounded-3xl flex items-center justify-center">
                                  <button type="button" onClick={() => setFormData({ ...formData, secondImageUrl: '' })} className="px-4 py-2 bg-white text-black text-[9px] font-black uppercase tracking-widest rounded-full">Replace Target</button>
                                </div>
                              </div>
                            ) : (
                              <div className="relative h-[220px] border-2 border-dashed border-stone-200 rounded-[32px] flex flex-col items-center justify-center gap-4 hover:border-[#e3ae97] transition-colors cursor-pointer bg-stone-50/50">
                                <ImageIcon className="text-stone-300" size={32} />
                                <span className="text-[9px] font-black text-stone-400 uppercase tracking-widest">Upload Modified Scene</span>
                                <input type="file" className="absolute inset-0 opacity-0 cursor-pointer" onChange={(e) => handleFileUpload(e.target.files[0], 'secondImageUrl')} />
                              </div>
                            )}
                          </div>
                        </div>
                ...
                <main className={`flex-1 transition-all duration-500 overflow-y-auto custom-scrollbar ${previewMode ? 'p-8 md:p-16' : 'p-12'}`}>
                  <div className={`mx-auto space-y-8 transition-all duration-500 ${previewMode ? 'max-w-[1600px]' : 'max-w-6xl'}`}>
                    ...
                    <div className="relative" onClick={handleImageClick}>
                      {formData.imageUrl ? (
                        <div className={`grid grid-cols-1 md:grid-cols-2 gap-6 items-start justify-items-center w-full`}>
                          ...
                          <div className="w-full">
                            <div className="relative rounded-[40px] overflow-hidden shadow-2xl border border-white group hover:shadow-rose-100/50 transition-all duration-500">
                              <img src={resolveImageUrl(formData.secondImageUrl || formData.imageUrl)} alt="Target" className="w-full h-auto block" draggable="false" />
