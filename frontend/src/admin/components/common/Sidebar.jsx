  ...
  return (
    <div className="w-[280px] lg:w-64 h-full bg-[#fdfbf7] lg:bg-white/55 lg:backdrop-blur-[14px] rounded-none lg:rounded-[24px] border-r border-[#e6e3df] lg:border-white/40 px-6 py-10 flex flex-col shadow-2xl lg:shadow-[0_4px_15px_rgba(0,0,0,0.03)] relative z-50">
      ...
      <div className="mt-auto px-2 mb-4">
        <button onClick={() => { if (window.confirm("Log out?")) { if (socketRef.current) { socketRef.current.disconnect(); socketRef.current = null; } logout(); } }} className="w-full flex items-center gap-3 px-4 py-3 rounded-[14px] text-[11px] font-medium uppercase tracking-widest transition-all duration-200 text-[#5f5f5f] hover:bg-red-50 hover:text-red-500 group">
